...
**결정 (D-019)**: **1차 소스는 서울 `vPetInfo`.** 국가 API는 1차 소스에서 내리고 전국 확장 경로로만 보존한다.

이유는 성격 데이터다. 국가 API의 `specialMark`로는 D-03 주석("AI 소개문은 공고 원본 필드만 근거, 없는 사실 창작 금지")을 지키면서 소개문을 만들 재료가 안 나온다 — 실제로 "관리번호 6-088M" 같은 값이 들어온다. 서울은 마리당 1,800~8,300자의 성격 서술과 `FOSTER_STATUS`(임시보호가능)를 준다. 후자는 §4.4 참여 퍼널의 임보 단계와 연결되는 유일한 필드다.

대가는 물량이다 — **24마리(개 15·고양이 9, 입양문의가능 11), 서울 한정.** 발표에서 물량 질문이 나오면 아래 §1의 국가 API 조사 결과가 "전국 4,521건 확장 가능, 단 성격은 AI 보완 전제"라는 답의 근거가 된다.
